...
from typing import Optional, List, Dict, Hashable, Callable, Union, Tuple
...
class Vocab:
    ...
    Args:
        list_of_tokens: list of tokens when pre-built vocab exists
        counter:
    ...
    """
    def __init__(self, list_of_tokens: List[str] = None,
                 counter: Optional[Counter] = None, max_size: Optional[int] = None,
        ...
        if list_of_tokens:
            self._idx_to_token.extend(
                [token for token in list_of_tokens if token not in self._idx_to_token])
            self._token_to_idx.update((token, idx) for idx, token in enumerate(self._idx_to_token))
...
class Tokenizer:
    ...
    def split(self, sent:str):
        """ sentence split by split function """
        return self._split_fn(sent)
...
class PreProcessor():
    ...
    def __init__(self, tokenizer:Tokenizer, max_seq_length:int, pad=True, pair=False):
        self._tokenizer = tokenizer
        self._max_seq_length = max_seq_length
        self._pad = pad
        self._pair = pair

    def preprocess(self, string:Union[Tuple[str], str]):
        if self._pair:
            assert len(string) == 2
            text_a = string[0]
            text_b = string[1]
        ...
        tokens_a = self._tokenizer.split(text_a)
        tokens_b = None

        if self._pair:
            tokens_b = self._tokenizer.split(text_b)
        ...
        input_ids = self._tokenizer.transform(tokens)
        ...
        if self._pad:
            ...
            input_ids.extend([self._tokenizer.vocab.token_to_idx['[PAD]']] * padding_length)  # 확인할 것
